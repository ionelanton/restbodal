(function (factory) {
  ...
  // <tt>Sammy.Hogan</tt> provides a quick way of using hogan.lib style templates in your app.
  // The plugin wraps the awesome hogan.lib lib created and maintained by Twitter
  // at http://twitter.github.com/hogan.lib/
  //
  // Note: As of Sammy 0.7 the Hogan.lib lib is not included in the templates source. Please download
  // hogan.lib and include it before Sammy.Hogan.
  //
  // Hogan.lib is a clever templating system that relys on double brackets {{}} for interpolation.
  // For full details on syntax check out the documentation at
  // http://twitter.github.com/hogan.lib/
  //
  ...
  //
  // Note: You dont have to include the hogan.lib file on top of the plugin as the plugin
  // includes the full source.
  //
  Sammy.Hogan = function(app, method_alias) {
    var cached_templates = {};

    // *Helper* Uses Hogan.lib to parse a template and interpolate and work with the passed data
    //
    // ### Arguments
    //
    // * `template` A String template. {{}} Tags are evaluated and interpolated by Hogan.lib
